...
if(total_slides==1){if(settings.singleSlideNavDisable){settings.automaticSlide=!1;settings.showControls=!1;settings.showNavigation=!1;settings.showProgressBar=!1}else{var clone=getSlide(0);var prepend=SLIDER.find(CRELLY).find(SLIDES);clone.clone().prependTo(prepend);total_slides++}}
...
getSlides().find('.cs-background-link').html(' ').data({'left':0,'top':0,'in':'none','out':'none','easeIn':0,'easeOut':0,'delay':0,'time':'all',});setPreloader();initVideos().done(function(){var loading=setInterval(function(){if(document.readyState=='complete'&&SLIDER.find(CRELLY).find('.cs-preloader').length>0){clearInterval(loading);loadedWindow()}},100)})}
...
if(e.data===YT.PlayerState.PLAYING){if(getItemData(element,'pause-while-watching')){prevent_hover_interactions=!0;pause()}}
else if(e.data===YT.PlayerState.PAUSED||e.data===YT.PlayerState.ENDED){if(!watchingAndWait()){prevent_hover_interactions=!1;resume()}}},},});temp={player:player,played_once:!1,manually_paused:!1,};youtube_videos[element.attr('id')]=temp})});return def.promise()}
...
function addListeners(){if(settings.responsive){$(window).resize(function(){if(window_width_before_setResponsive!=$(window).width()&&((settings.layout=='full-width'&&getWidth()!=$(SLIDER).width())||($(SLIDER).width()<getWidth()||(($(SLIDER).width()>getWidth())&&getWidth()<settings.startWidth)))){setResponsive()}})}
SLIDER.find(CRELLY).find('.cs-controls > .cs-previous').click(function(){changeSlide(getPreviousSlide())});SLIDER.find(CRELLY).find('.cs-controls > .cs-next').click(function(){changeSlide(getNextSlide())});if(settings.enableSwipe){SLIDER.find(CRELLY).on('swipeleft',function(){resume();changeSlide(getNextSlide())});SLIDER.find(CRELLY).on('swiperight',function(){resume();changeSlide(getPreviousSlide())})}
SLIDER.find(CRELLY).find('.cs-navigation > .cs-slide-link').click(function(){changeSlide($(this).index())});if(settings.pauseOnHover){SLIDER.find(CRELLY).find(SLIDES).hover(function(){if(prevent_hover_interactions==0){pause()}});SLIDER.find(CRELLY).find(SLIDES).mouseleave(function(){if(prevent_hover_interactions==0){resume()}})}}
...
return current_slide-1}
function getItemData(item,data){var is_slide;if(item.parent('ul').hasClass('cs-slides')){is_slide=!0}
else{is_slide=!1}
...
break;case 'ignore-ease-out':if(parseInt(item.data(data))==1){return!0}
else if(parseInt(item.data(data))==0){return!1}
return settings.ignoreElementsEaseOut;break;case 'autoplay':if(parseInt(item.data(data))==1){return!0}
else if(parseInt(item.data(data))==0){return!1}
return settings.videoAutoplay;break;case 'loop':if(parseInt(item.data(data))==1){return!0}
else if(parseInt(item.data(data))==0){return!1}
return settings.videoLoop;break;case 'start-mute':if(parseInt(item.data(data))==1){return!0}
else if(parseInt(item.data(data))==0){return!1}
return settings.videoStartMute;break;case 'pause-while-watching':if(parseInt(item.data(data))==1){return!0}
else if(parseInt(item.data(data))==0){return!1}
...
this.getTotalSlides=function(){return total_slides}};$.fn.crellySlider=function(options){var settings=$.extend({layout:'fixed',responsive:!0,startWidth:1140,startHeight:500,pauseOnHover:!0,automaticSlide:!0,randomOrder:!0,startFromSlide:0,showControls:!0,showNavigation:!0,singleSlideNavDisable:!1,showProgressBar:!0,enableSwipe:!0,slidesTime:3000,elementsDelay:0,elementsTime:'all',slidesEaseIn:300,elementsEaseIn:300,slidesEaseOut:300,elementsEaseOut:300,ignoreElementsEaseOut:!1,videoAutoplay:!1,videoLoop:!1,videoStartMute:!1,videoPauseWhileWatching:!0,beforeStart:function(){},beforeSetResponsive:function(){},beforeSlideStart:function(){},beforePause:function(){},beforeResume:function(){},},options);return this.each(function(){if(undefined==$(this).data('crellySlider')){var plugin=new $.CrellySlider(this,settings);$(this).data('crellySlider',plugin)}})}})(jQuery)
this.getTotalSlides=function(){return total_slides}};$.fn.crellySlider=function(options){var settings=$.extend({layout:'fixed',responsive:!0,startWidth:1140,startHeight:500,pauseOnHover:!0,automaticSlide:!0,randomOrder:!0,startFromSlide:0,showControls:!0,showNavigation:!0,singleSlideNavDisable:!1,showProgressBar:!0,enableSwipe:!0,slidesTime:3000,elementsDelay:0,elementsTime:'all',slidesEaseIn:300,elementsEaseIn:300,slidesEaseOut:300,elementsEaseOut:300,ignoreElementsEaseOut:!1,videoAutoplay:!1,videoLoop:!1,videoStartMute:!1,videoPauseWhileWatching:!0,beforeStart:function(){},beforeSetResponsive:function(){},beforeSlideStart:function(){},beforePause:function(){},beforeResume:function(){},},options);return this.each(function(){if(undefined==$(this).data('crellySlider')){var plugin=new $.CrellySlider(this,settings);$(this).data('crellySlider',plugin)}})}})(jQuery)
